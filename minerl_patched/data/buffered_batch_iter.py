...
import numpy as np
from minerl_patched.data.util import multimap
import random
...
class BufferedBatchIter:
    ...
    def optionally_fill_buffer(self):
        ...
        while (self.buffer_target_size - len(self.data_buffer)) > self.avg_traj_size:
            ...
            # note: include_metadata=True is specific to iglu
            data_loader = self.data_pipeline.load_data(traj_to_load, include_metadata=True, verbose=False)
            traj_len = 0
    ...
    def get_batch(self, batch_size, include_metadata=True):
        ...
        for _ in range(batch_size):
            ...
            if include_metadata and len(data_tuple) > 4:
                ret_dict['meta'] = data_tuple[5]
            ret_dict_list.append(ret_dict)
        return multimap(stack, *ret_dict_list)

    def buffered_batch_iter(self, batch_size, num_epochs=None, num_batches=None, include_metadata=True):
        """
        ...
        ** You must specify one or the other **
        ...
        while True:
            ...
            ret_batch = self.get_batch(batch_size=batch_size, include_metadata=include_metadata)
            batch_count += 1
